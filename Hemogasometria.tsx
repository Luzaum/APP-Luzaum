        ...
        <div className="container mx-auto p-4 md:p-8 max-w-4xl">
            ...
            <div className="mb-6 border-b border-gray-200">
                <nav className="flex space-x-4" aria-label="Tabs">
                    <button className={`tab-button font-medium py-3 px-4 border-b-2 border-transparent text-gray-900 hover:text-gray-900 hover:border-indigo-600 ${activeTab === 'analyzer' ? 'active' : ''}`} onClick={() => setActiveTab('analyzer')}>Analisador Clínico</button>
                    <button className={`tab-button font-medium py-3 px-4 border-b-2 border-transparent text-gray-900 hover:text-gray-900 hover:border-indigo-600 ${activeTab === 'quiz' ? 'active' : ''}`} onClick={() => setActiveTab('quiz')}>🧠 Modo Quiz</button>
                </nav>
            </div>
                    ...
                    {showResults && analysisResult && (
                            ...
                            {alerts.length > 0 && (
                                <div className="mt-8 space-y-3">
                                    ...
                                    {alerts.map((alert, index) => {
                                        const colors = { critical: 'bg-red-100 border-red-500 text-gray-900', warning: 'bg-yellow-100 border-yellow-500 text-gray-900' };
                                        return (
                     ...
                     <div className="mt-8">
                        <details className="bg-gray-100 border border-gray-200 rounded-lg">
                            <summary className="font-semibold p-3 cursor-pointer text-gray-900 hover:bg-gray-200 rounded-t-lg">📖 Guia de Boas Práticas de Coleta</summary>
                            <div className="p-4 border-t border-gray-300 space-y-3 text-sm text-gray-900">
            ...
            {activeTab === 'quiz' && quizCase && (
                <div className="bg-white p-6 rounded-2xl shadow-lg">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-bold text-gray-900">Caso Clínico Interativo</h2>
                        <button onClick={handleNewQuizCase} className="bg-indigo-100 text-gray-900 font-semibold py-2 px-4 rounded-lg hover:bg-indigo-200 transition">Gerar Novo Caso</button>
                    </div>
                ...
                <div className="modal-overlay visible" onClick={() => setModalData(null)}>
                    <div className="modal-content" onClick={(e) => e.stopPropagation()}>
                        <div className="flex justify-between items-center mb-4">
                            ...
                            <button onClick={() => setModalData(null)} className="text-gray-900 hover:text-gray-800 text-2xl">&times;</button>
                        </div>
        ...
        <div className={`result-card bg-white p-5 rounded-xl shadow-md border border-gray-200 flex items-start space-x-4 visible`} style={{transitionDelay: `${delay}ms`}}>
            ...
            <div className="flex-grow">
                ...
                {electrolyteStatus.map(e => {
                    const colorClass = 'text-gray-900 font-bold';
                    return (
                        <div key={e.name} className="py-1 flex items-center justify-between text-gray-900">
                            <span><strong>{e.name}:</strong> {e.value} {e.unit} <span className={colorClass}>({e.status})</span> <small className="text-gray-900">Ref: {e.ref}</small></span>
