...
const Modal = ({ id, title, children, onClose }) => (
    <div id={id} className="modal" style={{ display: 'flex' }} role="dialog" aria-modal="true" aria-labelledby={`${id}Title`} onClick={onClose}>
        <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 id={`${id}Title`} className="text-xl font-bold mb-4 text-gray-800">{title}</h3>
            {children}
        </div>
    </div>
);
...
const Fluidoterapia = ({ onBack }: { onBack: () => void }) => {
        ...
        return (
            <div className="final-rate-card mt-4">
                ...
                <div className="text-center my-2">
                    <span className="final-rate-value text-blue-700">{rateMlHr.toFixed(1)}</span>
                    <span className="text-blue-600 font-semibold">mL/hora</span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-center text-sm text-gray-700">
                ...
                <p className="text-xs text-center text-gray-600 mt-2">{description}</p>
            </div>
        );
    };
    ...
    const hipertonicaResults = useMemo(() => {
        ...
        if (base === 'flaconete') {
            ...
            preparoHtml = `<div class="result-row"><span class="result-row-label">Receita</span><span class="result-row-value">1 mL NaCl 20% para ${ratio.toFixed(1)} mL NaCl 0.9%</span></div><p class="text-xs text-center text-gray-500 pt-2">Misture na proporção indicada para atingir ${target}%.</p>`;
        } else {
            const baseVolume = parseFloat(base);
            let mlToAdd = (target === 3) ? (baseVolume / 100) * 12.35 : (baseVolume / 100) * 52.8;
            preparoHtml = `<div class="result-row"><span class="result-row-label">1. Remover da bolsa</span><span class="result-row-value">${mlToAdd.toFixed(1)} mL de NaCl 0.9%</span></div><div class="result-row"><span class="result-row-label">2. Adicionar à bolsa</span><span class="result-row-value">${mlToAdd.toFixed(1)} mL de NaCl 20%</span></div><p class="text-xs text-center text-gray-500 pt-2">(Equivalente a ${(mlToAdd / 10).toFixed(1)} ampolas de 10mL)</p>`;
        }

        let adminHtml = '';
        if (isNaN(p) || p <= 0) {
            adminHtml = `<p class="text-center text-gray-500">Insira o peso do paciente.</p>`;
        } else {
            const dose = parseFloat(doseHipertonica);
            const volumeTotal = p * dose;
            const taxaInfusao = volumeTotal / (20 / 60); // Padrão de 20 min
            adminHtml = `<div class="result-row"><span class="result-row-label">💉 Volume a Administrar</span><span class="result-row-value">${volumeTotal.toFixed(1)} mL</span></div><div class="result-row"><span class="result-row-label">⏱️ Taxa de Infusão</span><span class="result-row-value">${taxaInfusao.toFixed(1)} mL/h</span></div><p class="text-xs text-center text-gray-500 pt-2">(Administrar em 15-20 minutos)</p>`;
        }
        return { preparoHtml, adminHtml };
    }, [peso, targetHipertonica, baseHipertonica, doseHipertonica]);
    
    const HelpIcon = ({ modalId, fisiologiaId, ...props }: { modalId?: string; fisiologiaId?: string; }) => (
        <span
            className="help-icon"
            role="button"
            ...
            onClick={(e) => {
                ...
                if (fisiologiaId) setActiveModal(fisiologiaId);
            }}
            {...props}
        >
            ?
    ...
    const renderInfoCard = (type, data) => {
      if (!type || type === 'nenhuma' || !data) return <p className="text-center text-gray-500">Selecione uma opção para ver as recomendações.</p>;
      if(type === 'saudavel') {
    ...
    return (
        <>
            <style>{`
                ...
                .input-label { display: block; font-weight: 600; color: #4a5568; margin-bottom: 0.5rem; }
                .input-field { width: 100%; padding: 0.75rem; border: 1px solid #cbd5e0; border-radius: 0.5rem; transition: border-color 0.2s, box-shadow 0.2s; background-color: #fff; color: #2d3748; }
                ...
                .modal-content { background-color: #fefefe; margin: auto; padding: 2rem; border: 1px solid #888; width: 90%; max-width: 600px; border-radius: 1rem; box-shadow: 0 5px 15px rgba(0,0,0,0.3); position: relative; }
                .close-button { color: #555; position: absolute; right: 1rem; top: 1rem; font-size: 28px; font-weight: bold; cursor: pointer; line-height: 1; }
                ...
                .collapsible-header { cursor: pointer; display: flex; justify-content: space-between; align-items: center; padding: 1rem; background-color: #e2e8f0; border-radius: 0.5rem; font-weight: 600; color: #2d3748; }
                ...
                .species-btn.selected { border-color: #3b82f6; background-color: #dbeafe; color: #1e40af; }
            `}</style>
             {activeModal && (
                <Modal id={activeModal} title={fisiologiaData[activeModal]?.title || 'Guia'} onClose={() => setActiveModal(null)}>
                    {activeModal === 'modalDesidratacao' && <table className="w-full text-sm text-left text-gray-500"><thead className="text-xs text-gray-700 uppercase bg-gray-100"><tr><th className="px-4 py-2">% Desid.</th><th className="px-4 py-2">Sinais Clínicos</th></tr></thead><tbody><tr className="bg-white border-b"><td className="px-4 py-2 font-medium">&lt; 5%</td><td className="px-4 py-2">Não detectável clinicamente. Histórico de perdas.</td></tr><tr className="bg-gray-50 border-b"><td className="px-4 py-2 font-medium">5-8%</td><td className="px-4 py-2">Leve perda de turgor cutâneo, mucosas pegajosas.</td></tr><tr className="bg-white border-b"><td className="px-4 py-2 font-medium">8-10%</td><td className="px-4 py-2">Perda moderada de turgor, mucosas secas, enoftalmia leve, TPC 2-2.5s.</td></tr><tr className="bg-gray-50 border-b"><td className="px-4 py-2 font-medium">10-12%</td><td className="px-4 py-2">Pele sem elasticidade, enoftalmia acentuada, TPC **{'>'}**2.5s, taquicardia, pulsos fracos.</td></tr><tr className="bg-white"><td className="px-4 py-2 font-medium">&gt; 12%</td><td className="px-4 py-2">Sinais de choque hipovolêmico, mucosas pálidas, TPC **{'>'}**3s, hipotermia.</td></tr></tbody></table>}
                    {activeModal === 'modalPerdas' && <><p className="mb-2">A forma mais precisa é pesar os dejetos (1g ≈ 1mL). Na impossibilidade, use estimativas:</p><ul className="list-disc list-inside text-gray-700 space-y-2"><li><strong>Vômito/Regurgitação:</strong><ul className="list-disc list-inside ml-4"><li>Pequeno porte: 10-30 mL por episódio.</li><li>Médio porte: 30-100 mL por episódio.</li><li>Grande porte: 100-300+ mL por episódio.</li></ul></li><li><strong>Diarreia Líquida:</strong><ul className="list-disc list-inside ml-4"><li>Pequeno porte: 20-50 mL por episódio.</li><li>Médio porte: 50-150 mL por episódio.</li><li>Grande porte: 150-400+ mL por episódio.</li></ul></li><li><strong>Poliúria:</strong> A única forma confiável é a mensuração com cateter urinário e sistema de bolsa coletora fechada.</li></ul><p className="mt-4 text-sm text-gray-600"><strong>Lembrete:</strong> São apenas estimativas. O monitoramento clínico e laboratorial do paciente é fundamental.</p></>}
                    {activeModal === 'modalNotasClinicas' && <><p className="mb-4 text-sm text-gray-600">A abordagem "low and slow" (baixa taxa e lentamente) é crucial para evitar sobrecarga de volume nestes pacientes.</p><table className="w-full text-sm text-left text-gray-500"><thead className="text-xs text-gray-700 uppercase bg-gray-100"><tr><th className="px-4 py-2">Condição de Risco</th><th className="px-4 py-2">Tempo de Reidratação</th><th className="px-4 py-2">Bolus de Ressuscitação</th></tr></thead><tbody><tr className="bg-white border-b"><td className="px-4 py-2 font-medium">❤️ Cardiopata</td><td className="px-4 py-2">18-24h (ou mais)</td><td className="px-4 py-2">2-5 mL/kg em 20-30 min</td></tr><tr className="bg-gray-50 border-b"><td className="px-4 py-2 font-medium">💧 Doente Renal (Oligúrico)</td><td className="px-4 py-2">18-24h</td><td className="px-4 py-2">5-10 mL/kg em 20-30 min</td></tr><tr className="bg-white border-b"><td className="px-4 py-2 font-medium">🐾 Filhote / Idoso</td><td className="px-4 py-2">18-24h</td><td className="px-4 py-2">5-10 mL/kg em 20-30 min</td></tr><tr className="bg-gray-50"><td className="px-4 py-2 font-medium">📉 Hipoalbuminemia</td><td className="px-4 py-2">18-24h</td><td className="px-4 py-2">Cristaloide (volume reduzido) + Coloide</td></tr></tbody></table></>}
                    {activeModal === 'modalTempoBolus' && <><p className="mb-4 text-sm text-gray-600">A velocidade de administração do bolus deve ser ajustada à condição do paciente.</p><ul className="list-disc list-inside text-gray-700 space-y-2"><li><strong>15-20 minutos:</strong> Tempo padrão para a maioria dos casos de choque hipovolêmico em pacientes sem comorbidades cardíacas ou renais.</li><li><strong>30 minutos (ou mais):</strong> Recomendado para pacientes de risco (cardiopatas, renais, filhotes, idosos) para permitir a reavaliação da tolerância ao volume e evitar sobrecarga hídrica aguda.</li></ul></>}
                    {fisiologiaData[activeModal] && <div dangerouslySetInnerHTML={{ __html: fisiologiaData[activeModal].content }} />}
                </Modal>
            )}
            <div className="p-4 sm:p-8">
                <div className="max-w-4xl mx-auto">
                    ...
                    <header className="text-center mb-8">
                        <h1 className="text-4xl font-bold text-gray-800">Calculadora de Fluidoterapia 💧</h1>
                        <p className="text-gray-600 mt-2">Cálculo de taxas para cães e gatos com base em diretrizes clínicas.</p>
                    </header>
                    <div className="card mb-8">
                        <h2 className="text-2xl font-bold text-gray-700 mb-4 text-center">1. Selecione a Espécie</h2>
                        <div className="flex justify-center gap-8">
                            <button id="btnCao" className={`species-btn ${especie === 'cao' ? 'selected' : ''}`} aria-pressed={especie === 'cao'} onClick={() => handleSelectSpecies('cao')}>🐶 Cão</button>
                            <button id="btnGato" className={`species-btn ${especie === 'gato' ? 'selected' : ''}`} aria-pressed={especie === 'gato'} onClick={() => handleSelectSpecies('gato')}>🐱 Gato</button>
                        </div>
                    </div>
                    {especie && (
                         <main id="mainCalculator">
                            <div className="card grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <h2 className="text-2xl font-bold text-gray-700 mb-6 border-b pb-2">2. Dados do Paciente</h2>
                                    ...
                                    <h2 className="text-2xl font-bold text-gray-700 my-6 border-b pb-2">3. Plano de Fluidoterapia</h2>
                                    <div className="input-group">
                                        <label htmlFor="taxaManutencao" className="input-label">Taxa de Manutenção (mL/kg/dia)</label>
                                        <div className="flex items-center space-x-4">
                                            <span className="font-mono">{manutencaoRange.min}</span>
                                            ...
                                            <span className="font-mono">{manutencaoRange.max}</span>
                                        </div>
                                ...
                                <div className="flex flex-col">
                                    <h2 className="text-2xl font-bold text-gray-700 mb-6 border-b pb-2">4. Plano de Infusão</h2>
                            ...
                            <Collapsible id="ressuscitacao" title="⚡ Ressuscitação Volêmica (Choque / Hipovolemia)">
                                <h3 className="text-lg font-semibold text-gray-700 mb-4">Calculadora de Bolus</h3>
                                <div className="input-group">
                                    <label htmlFor="taxaBolus" className="input-label">Dose de Bolus (mL/kg)</label>
                                    <div className="flex items-center space-x-4"><span className="font-mono">{bolusRange.min}</span><input type="range" id="taxaBolus" className="w-full" min={bolusRange.min} max={bolusRange.max} step="1" value={taxaBolus} onChange={e => setTaxaBolus(e.target.value)} /><span className="font-mono">{bolusRange.max}</span></div>
                                    <div className="text-center mt-2 font-semibold text-blue-600">Dose Selecionada: <span>{taxaBolus}</span> mL/kg</div>
                                </div>
                                <div className="input-group">
                                ...
                                <div className="input-group mt-6">
                                    <label htmlFor="comorbidade" className="input-label flex items-center">Selecione a Comorbidade <HelpIcon fisiologiaId={comorbidade} /></label>
                            ...
                            <Collapsible id="especificas" title="🔬 Fluidos para Condições Específicas">
                                <div className="input-group">
                                    <label htmlFor="condicaoEspecial" className="input-label">Selecione a condição clínica para ver as recomendações: <HelpIcon fisiologiaId={condicaoEspecial} /></label>
                            ...
                            <Collapsible id="hipertonica" title="🧪 Calculadora de Salina Hipertônica">
                                <h3 className="text-lg font-semibold text-gray-700 mb-4">1. Preparo da Solução</h3>
                                ...
                                <h3 className="text-lg font-semibold text-gray-700 my-4 pt-4 border-t">2. Cálculo de Administração</h3>
                                <div className="input-group"><label htmlFor="doseHipertonica" className="input-label">Dose (mL/kg)</label><div className="flex items-center space-x-4"><span className="font-mono">2</span><input type="range" id="doseHipertonica" className="w-full" min="2" max="5" step="0.5" value={doseHipertonica} onChange={e => setDoseHipertonica(e.target.value)} /><span className="font-mono">5</span></div><div className="text-center mt-2 font-semibold text-blue-600">Dose Selecionada: <span>{doseHipertonica}</span> mL/kg</div></div>
